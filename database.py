...
def get_all_events(user_id):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM events WHERE user_id=%s ORDER BY date", (user_id,)
            )
            return cur.fetchall()
...
def add_sub_event(event_id, name, contact=None, num_participants=None, participants=None, teacher_in_charge=None):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO sub_events (event_id, name, contact, num_participants, participants, teacher_in_charge) VALUES (%s,%s,%s,%s,%s,%s)",
                (event_id, name, contact, num_participants or None, participants, teacher_in_charge)
            )
...
def get_sub_events(event_id):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM sub_events WHERE event_id=%s", (event_id,)
            )
            return cur.fetchall()
...
def update_sub_event(sub_id, name, contact=None, num_participants=None, participants=None, teacher_in_charge=None):
    with get_db_connection() as conn:
        with conn.cursor() as cur:
            cur.execute(
                "UPDATE sub_events SET name=%s, contact=%s, num_participants=%s, participants=%s, teacher_in_charge=%s WHERE id=%s",
                (name, contact, num_participants or None, participants, teacher_in_charge, sub_id)
            )
